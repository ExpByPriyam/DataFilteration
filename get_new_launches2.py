...
# THIS ONLY RUNS FOR NEW DATA IF THIS IS TRUE - otherwise it uses last runs data
new_data = True
MARKET_CAP_MAX = 30000  # max market cap *ONLY APPLIES IF NEW_DATA = TRUE*
# number of tokens to search through on solana *ONLY APPLIES IF NEW_DATA = TRUE* i found 15000 gets a lot
NUM_TOKENS_2SEARCH = 15000
MIN_24HR_VOLUME = 1000  # min 24 hour volume *ONLY APPLIES IF NEW_DATA = TRUE*

# if the sell % more than MAX_SELL_PERCENTAGE sells in the last hour, drop it
MAX_SELL_PERCENTAGE = 70
# if there are less than MIN_TRADES_LAST_HOUR trades in the last hour, drop it
MIN_TRADES_LAST_HOUR = 9
# if there are less than MIN_UNQ_WALLETS2hr unique wallets in the last 2 hours, drop it
...
def birdeye_bot():
    ...
    while total_tokens < num_tokens:
        try:
            print(
                f'scanning solana for new tokens, total scanned: {total_tokens}...')
            params = {"sort_by": "v24hChangePercent",
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error {response.status_code}: trying again in 10 seconds...")
    ...
    df['lastTradeUnixTime'] = pd.to_datetime(
        df['lastTradeUnixTime'], unit='s').dt.tz_localize('UTC')

    # Calculate the current time in UTC
    current_time = datetime.datetime.now(datetime.timezone.utc)

    # Calculate the time 10 minutes ago in UTC
    ten_minutes_ago = current_time - \
        datetime.timedelta(minutes=mins_last_trade)
...
def token_overview(address, MAX_SELL_PERCENTAGE, MIN_TRADES_LAST_HOUR, MIN_UNQ_WALLETS2hr, MIN_VIEW24h, MIN_LIQUIDITY):
    ...
    if response.status_code == 200:
        ...
        links.extend([{'website': u}
                     for u in urls if 't.me' not in u and 'twitter.com' not in u])
